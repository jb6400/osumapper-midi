import re, os, subprocess, json, soundfile, pretty_midi
import numpy as np
...
def read_osu_file(path, convert=False, wav_name="wavfile.wav", midi_name="wavfile.midi",json_name="temp_json_file.json"):
    ...
    with open(json_name, encoding="utf-8") as map_json:
        ...
        if convert:
            ...
            subprocess.call([GLOBAL_VARS["waon_path"], "-i", wav_name, "-o",midi_name]);
...
def get_freqs(sig, fft_size):
    ...
    Lg = np.angle(Lc[0:fft_size//2],deg=True);
    return La, Lg;
